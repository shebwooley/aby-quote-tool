  ...
  products: {
    ...
    aca: {
      ...
      paragraphs: [
        ...
        'Which form applies depends on the plan, not only on employer size. The 1095-B is created for employers whose group health plan is level funded, self funded or balance funded, and only for employees covered under that plan. The 1095-C is for all Applicable Large Employers and is created for every benefit eligible employee.',
        'True Full Service includes a dedicated senior account manager, a discovery interview session, creation of Forms 1094 and 1095, 1095-C code calculation, form quality control checks, test data and data planning, data manipulation, print ready PDF delivery, and e-filing with the IRS. Self Service covers the filing itself and leaves the preparation and review with the employer.',
        'Form counts are based on the number of forms created and filed, not on current headcount: total the employees who were full time on any day of the calendar year. Pricing is per EIN, and employers with more than one EIN must take Full Service, as Self Service is not available in that case.',
        'Rhode Island, New Jersey, Massachusetts, California, Washington DC and Vermont require their own state filing for employees residing there. Most fully insured carriers file the state forms on the plan behalf, so ABY recommends confirming that with the carrier before electing the state filing service.',
        'If the IRS accepts forms with errors, ABY can correct and refile them. This is optional, but ABY strongly recommends that corrections are made and resubmitted, because an accepted-with-errors filing does not close out the reporting obligation.'
      ]
    },
